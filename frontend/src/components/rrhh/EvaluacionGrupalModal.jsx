...
const EvaluacionGrupalModal = ({ isOpen, onClose, onSave, evaluacion }) => {
  ...
  const loadCompetenciasEstandar = async () => {
    try {
      const competencias = await evaluacionesGrupalesService.getCompetenciasEstandar();
  ...
  const loadEvaluacionCompleta = async () => {
    try {
      setLoading(true);
      const evaluacionCompleta = await evaluacionesGrupalesService.getById(evaluacion.id);
      setFormData({
