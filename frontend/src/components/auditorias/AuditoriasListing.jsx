...
const AuditoriasListing = () => {
  ...
  const {
    ...
    goToPage,
    changeItemsPerPage,
  } = usePaginationWithFilters(auditorias, {
    itemsPerPage: viewMode === 'kanban' ? 50 : 12 // Más elementos para Kanban
  });
  ...
  const filteredAuditorias = useMemo(() => {
    return paginatedAuditorias.filter(auditoria => {
      // Filtro por estado
      const matchesEstado = filterEstado === 'todos' || 
        auditoria.estado?.toLowerCase() === filterEstado.toLowerCase();
      
      // Filtro por tipo
      const matchesTipo = filterTipo === 'todos' || 
        auditoria.tipo?.toLowerCase() === filterTipo.toLowerCase();
      
      // Filtro por área
      const matchesArea = filterArea === 'todos' || 
        auditoria.area?.toLowerCase() === filterArea.toLowerCase();
      
      return matchesEstado && matchesTipo && matchesArea;
    });
  }, [paginatedAuditorias, filterEstado, filterTipo, filterArea]);

  // Obtener opciones únicas para los filtros
  const filterOptions = useMemo(() => {
    const estados = [...new Set(auditorias.map(a => a.estado).filter(Boolean))];
    const tipos = [...new Set(auditorias.map(a => a.tipo).filter(Boolean))];
    const areas = [...new Set(auditorias.map(a => a.area).filter(Boolean))];
    
    return { estados, tipos, areas };
  }, [auditorias]);

  const getAuditoriasByEstado = (estado) => {
    return filteredAuditorias.filter(audit => audit.estado === estado);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Búsqueda y Filtros */}
      <div className="bg-white rounded-lg shadow-sm border border-sgc-200 p-4 space-y-4">
        {/* Barra de búsqueda */}
        <div className="flex items-center gap-4">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
            <Input
              placeholder="Buscar auditorías por código, título, responsable..."
              value={searchTerm}
      ...
      {/* Paginación solo para vistas grid y list (Kanban maneja muchos elementos) */}
      {viewMode !== 'kanban' && !loading && paginationInfo.totalPages > 1 && (
        <div className="mt-6">
          <Pagination
